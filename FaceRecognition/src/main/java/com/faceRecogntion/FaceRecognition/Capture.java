...
public class Capture {
	...
	public void capture() throws Exception, InterruptedException {
		...
		data.put("Name", name);
		try {
			FileWriter writer = new FileWriter(basePath + "/src/main/resources/namedata.csv", true);
		...
		while ((capturedFrame = camera1.grab()) != null && cFrame.isVisible()) {
			while (sample <= sampleNumber) {
				...
				opencv_imgproc.cvtColor(colorImg, gray, opencv_imgproc.COLOR_BGRA2GRAY); // convert image to gray for
																							// better detection
				...
				opencv_imgcodecs.imwrite(
						basePath + "/src/main/resources/photos/" + name + "." + personId + "." + sample + ".jpg",
						captface);
